...
function setup_sections() {
	...
	add_settings_section( 'frontend_cron_section', 'Setup time for cron', 'section_callback', 'studio24_preview' );
}
...
function setup_fields() {
	add_settings_field( 'frontend_url_field', 'URL', 'url_field_callback', 'studio24_preview', 'frontend_url_section' );
	add_settings_field( 'frontend_cron_field', 'CRON', 'cron_field_callback', 'studio24_preview', 'frontend_cron_section' );
	register_setting( 'studio24_preview', 'frontend_url_field' );
	register_setting( 'studio24_preview', 'frontend_cron_field' );
}

function url_field_callback( $arguments ) {
	echo '<input name="frontend_url_field" id="frontend_url_field" type="url" value="' . get_option( 'frontend_url_field' ) . '" />';
}

function cron_field_callback( $arguments ) {
