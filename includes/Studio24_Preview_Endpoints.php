...
class Studio24_Preview_Endpoints {
	...
	private function register_endpoints() {
		add_action( 'rest_api_init', function () {
			register_rest_route( 'preview-studio-24/v1', '/new', array(
				'methods'  => 'GET',
		...
		add_action( 'rest_api_init', function () {
			register_rest_route( 'preview-studio-24/v1', '/revision/(?P<token>[\d\w]+)', array(
				'methods'  => 'GET',
	...
	function generate_token_and_redirect( $request ) {
		...
		if ( $actual_post_type !== $expected_post_type ) {
			return new WP_Error( 'post_type_not_matching', 'Invalid post type', array(
				'status' => 400,
				'text'   => 'Bad Request'
			) );
		}
